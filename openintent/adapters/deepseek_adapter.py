"""DeepSeek provider adapter for automatic OpenIntent coordination.

This adapter wraps the DeepSeek client (OpenAI-compatible) to automatically log
intent events for chat completions, tool calls, and streaming responses.

Note: DeepSeek uses an OpenAI-compatible API, so this adapter is similar to OpenAIAdapter
but configured for DeepSeek's endpoints.
...
def _check_deepseek_installed() -> None:
    """Check if the openai package is installed (DeepSeek uses OpenAI-compatible API)."""
    try:
        import openai  # noqa: F401
    except ImportError:
        raise ImportError(
            "DeepSeekAdapter requires the 'openai' package (DeepSeek uses OpenAI-compatible API). "
            "Install it with: pip install openintent[deepseek]"
        ) from None
...
class DeepSeekChat:
    ...
    def __init__(self, adapter: "DeepSeekAdapter"):
        self.completions = DeepSeekChatCompletions(adapter)


class DeepSeekAdapter(BaseAdapter):
    """Adapter for the DeepSeek API (OpenAI-compatible client).

    Wraps an OpenAI client configured for DeepSeek's API to automatically log
    ...
    def __init__(
        ...
        Args:
            deepseek_client: The OpenAI client configured for DeepSeek's API.
