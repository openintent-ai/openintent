...
async def demo_basic_auth():
    ...
    for api_key, agent_id in dev_keys.items():
        ...
        try:
            ...
            _intents = await client.list_intents(limit=1)
...
async def demo_multi_agent_identity():
    ...
    _orchestrator_creds = registry.register("coordinator", AgentRole.ORCHESTRATOR)
    _research_creds = registry.register("research-agent", AgentRole.WORKER)
    _synth_creds = registry.register("synthesis-agent", AgentRole.WORKER)
    ...
    orchestrator = AuthenticatedAgent(
        AgentCredentials(
    ...
    try:
        ...
        for event in events:
            ...
            _agent = payload.get("agent_id", payload.get("message", "N/A"))
